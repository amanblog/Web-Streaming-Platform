...
@section('content')
    <div class="row">
        <div class="col-md-6">
            <h3>{{$song->title}}</h3>
            <p><strong>Artist(s): </strong>@foreach($song->artists as $artist){{$artist->name}}@if($loop->last)@else,@endif @endforeach</p>
                        ...
                        <div class="audio-player">
                            <div class="player">
                                ...
                                <div class="controller">
                                    <img src="{{ url('storage/images/player/backward.png')  }}" width="30px" onclick="decreasePlaybackRate()">
                                    <img src="{{ url('storage/images/player/play.png')  }}" width="40px" onclick="playOrPauseSong(this)">
                                    <img src="{{ url('storage/images/player/forward.png')  }}" width="30px" onclick="increasePlaybackRate()">
                                    ...
                                    <img src="{{ url('storage/images/player/volume_up.png')  }}" width="30px">
                                </div>
                            </div>
                        </div>
        </div>
...
@section('scripts')
        ...
        <script>
            ...
            function loadSong(){
                ...
                song.src = "{{ route('audio',str_replace('public/','',$song->path)) }}";
                song.playbackrate=1;
                song.currentTime = 0;
            ...
            function playOrPauseSong(img){
                ...
                if (song.paused) {
                    song.play();
                    img.src=window.location.origin+"/storage/images/player/pause.png";
                }
                else{
                    song.pause();
                    img.src=window.location.origin+"/storage/images/player/play.png";
                }
            }
